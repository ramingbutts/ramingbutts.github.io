...
- **Observability** — `js/diag.js` centralizes structured logging in an in-memory ring buffer; storage-quota and Supabase-sync failures (previously silent) now log and surface a toast. View them in the **Diagnostics** page (sidebar) — event log, error/warning counts, sync status, and storage usage — or via `Diag.dump()` in the console
